...
namespace AdventOfCode.Day8.Puzzle2
{
    public static class Puzzle
    {
        public static void Solution()
        {
            ...
            var line1 = File.ReadLines(@"..\..\..\..\AdventOfCode\Day8\Puzzle2\input.txt");
            int[,] input = new int[line1.Count(), line1.Count()];
            int j = 0;
            foreach (string line in File.ReadLines(@"..\..\..\..\AdventOfCode\Day8\Puzzle2\input.txt"))
            {
            ...
            for (int i = 0; i < input.GetLength(0); i++)
            {
                ...
                {
                    int rightScore = 0;
                    if (nextGreaterRightRow[i,k] > 0)
                    {
                        rightScore=nextGreaterRightRow[i, k] - k;
                    }
                    else
                    {
                        rightScore=input.GetLength(0)-k-1;
                    }
                    int leftScore = 0;
                    if (nextGreaterLeftRow[i,k]>0)
                    {
                        leftScore = k - nextGreaterLeftRow[i, k];
                    }
                    else
                    {
                        leftScore = k;
                    ...
                    int aboveScore = 0;
                    if (aboveGreaterTranspose[i,k] > 0)
                    {
                        aboveScore=i- aboveGreaterTranspose[i, k];
                    }
                    else
                    {
                        aboveScore=i;
                    ...
                    int belowScore = 0;
                    if (belowGreaterTranspose[i,k]>0)
                    {
                        belowScore = belowGreaterTranspose[i, k] - i;
                    }
                    else
                    {
                        belowScore = input.GetLength(0) - i - 1;
                    }

                    int scenicScore=rightScore*leftScore*aboveScore*belowScore;
        ...
        static void findNextGreaterRightRow(int[,] nextGreaterRightRow, int[,] input, int rowIndex)
        {
            ...
            for (int k = 0; k < input.GetLength(0); k++)
            {
                while (heights.Count() > 0 && input[rowIndex, heights.Peek()] <= input[rowIndex, k])
                {
                    ...
                    nextGreaterRightRow[rowIndex, index] = k;
                }
                heights.Push(k);
            }
        }
        ...
        static void findNextGreaterLeftRow(int[,] nextGreaterLeftRow, int[,] input, int rowIndex)
        {
            ...
            for (int k = input.GetLength(0) - 1; k >= 0; k--)
            {
                ...
                {
                    int index = heights.Pop();
                    nextGreaterLeftRow[rowIndex, index] = k;
                }
                heights.Push(k);
            }
        }
